...
mod tests {
    ...
    fn renders_nested_types() {
        let typ = ObjectType::Option(Box::new(ObjectType::Map(
            Box::new(ObjectType::U16),
            Box::new(ObjectType::Vec(Box::new(ObjectType::String))),
        )));
    ...
    #[test]
    fn renders_initial_values() {
        let typ = ObjectType::Struct(
            "Point".to_string(),
            vec![
                ("x".to_string(), ObjectType::F32),
                ("label".to_string(), ObjectType::String),
            ],
        );
        let init = InitValue::Struct(
            "Point",
            vec![
                ("x", InitValue::F32(1.5)),
                ("label", InitValue::String("left".to_string())),
            ],
        );
    ...
    #[test]
    fn renders_map_initial_values_deterministically() {
        let typ = ObjectType::Map(Box::new(ObjectType::U8), Box::new(ObjectType::String));
        let init = InitValue::Map(vec![
            (InitValue::U8(2), InitValue::String("two".to_string())),
            (InitValue::U8(1), InitValue::String("one".to_string())),
        ]);

        assert_eq!(
            init_to_rust_value(&init, &typ),
            "std::collections::HashMap::<u8, String>::from([(1u8, String::from(\"one\")), (2u8, String::from(\"two\"))])"
        );
    }

    #[test]
    fn renders_non_finite_float_values() {
        assert_eq!(
            init_to_rust_value(&InitValue::F32(f32::NAN), &ObjectType::F32),
            "f32::NAN"
        );
        assert_eq!(
