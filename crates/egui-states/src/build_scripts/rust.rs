...
fn state_field_doc(state: &StateType) -> &'static str {
    match state {
        StateType::Value(..) => "Bidirectional synchronized value.",
        ...
        StateType::Data(..) => "Server-controlled numeric buffer.",
        StateType::DataTake(..) => "One-shot numeric buffer consumed by the client.",
        StateType::DataMulti(..) => "Server-controlled keyed numeric buffers.",
        StateType::DataMultiTake(..) => "Keyed one-shot numeric buffers consumed by the client.",
        StateType::SubState(..) => "Nested group of synchronized states.",
    }
}
...
fn write_state(
    ...
    writeln!(
        output,
        "\n/// Generated server handles for the `{state_class}` state group.\n#[derive(Clone)]\npub struct {state_class} {{"
    )
    .unwrap();
    for state in states {
        writeln!(
...
fn render_rust<S: State>() -> Result<(String, String, String), String> {
    ...
    let mut enums_output = String::from(
        "//! Custom enum types used by the generated state server.\n\n// Generated by build.rs, do not edit\n",
    );
    for (enum_name, variants) in &enums {
        writeln!(
            enums_output,
            "\n/// Generated protocol enum `{enum_name}`.\n#[egui_states::typed]\n#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]\npub enum {enum_name} {{"
        )
        .unwrap();
        for (variant, value) in variants {
            writeln!(
                enums_output,
                "    /// Protocol variant with discriminant `{value}`.\n    {variant} = {value},"
            )
            .unwrap();
        }
    ...
    let mut structs_output = String::from(
        "//! Custom struct types used by the generated state server.\n\n// Generated by build.rs, do not edit\n",
    );
    for struct_name in &struct_order {
        let fields = &structs[struct_name];
        writeln!(
            structs_output,
            "\n/// Generated protocol struct `{struct_name}`.\n#[egui_states::typed]\n#[derive(Clone, Debug, PartialEq)]\npub struct {struct_name} {{"
        )
        .unwrap();
    ...
    let mut states_output = String::from(
        "//! Typed server bindings generated from the egui client state tree.\n\n// Generated by build.rs, do not edit\n/// Custom enum types used by the state tree.\npub mod enums;\n/// Custom struct types used by the state tree.\npub mod structs;\n\nuse egui_states::server as s;\n",
    );
    ...
    writeln!(
        states_output,
        "\n/// Generated handles for the root `{root_name}` state group.\n#[derive(Clone)]\npub struct {root_name} {{"
    )
    .unwrap();
    for state in substates {
        writeln!(
    ...
    writeln!(
        states_output,
        "\nimpl {root_name} {{\n    /// Registers the complete state tree with `server`.\n    pub fn new(server: &s::StateServer) -> s::Result<Self> {{\n        let parent = \"root\";\n        Ok(Self {{"
    )
    .unwrap();
    for state in substates {
        let field = state.name();
